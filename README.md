...
lastReviewedNote: 'Reviewed for CLI #362: 0.1.21 is the version-only release preparation for merged source PRs #360 and #361; the package-version line separates the in-repository preparation from the actually published 0.1.20, no cli-v0.1.21 tag exists yet, and merge/publication stays held for the Database #674/#680 capability.'
---
...
Review note, 2026-09-22: Issue #362 prepares the 0.1.21 version-only release for merged source PRs #360 (CLI #358) and #361 (CLI #359) on branch `codex/issue-362-cli021` (base main merge `53a53b8`). Package identity and the live CLI-version fixtures advance; dependencies, the sole lock, runtime files, strict exports, commands and release automation stay unchanged. The source is merged but publication has not happened — npm latest is `0.1.20` and no `cli-v0.1.21` tag exists — so the package-version line above states the in-repository release preparation and the actually published version separately instead of claiming a release, and merge/publication stays held until the Database #674/#680 capability and current-closure guards reach qualified main and production deployment.

  ...
  assert.deepEqual(
    exceptions.filter(
      (entry) =>
        ...
        entry !== '@tiangong-lca/tidas-sdk@0.2.2',
    ),
    [],
...
test('the latest Node 24-compatible graph pins the released TIDAS 0.2.2 runtime', () => {
  const sdkRange = PACKAGE_JSON.dependencies?.['@tiangong-lca/tidas-sdk'];
  assert.equal(sdkRange, '0.2.2');
  assert.equal(compareVersions(firstVersion(sdkRange), [0, 2, 2]), 0);

...
`process-payload-validation.ts` owns Process schema, authoring and content checks for dataset validation, save-draft and BuildPlan reports. SDK validation uses a clone; the report hash binds the complete original candidate. `process-required-fields.ts` preserves unknown annual evidence and all valid language values, removes only exact historical annual sentinels, and retains the authoring hold. BuildPlan never derives actual annual production from a quantitative reference. Save/attempt ownership and numerical weight policy stay in their existing owners.

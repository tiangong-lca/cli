...
lastReviewedNote: 'Reviewed for CLI #350 after 0.1.19 main integration: the existing review compatibility layer bridges ordered Process arrays through SDK 0.3.0 without adding schema authority, dependency, export, command, ledger transport or ownership boundary.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
Review note, 2026-09-21: CLI #283 adds `src/lib/dataset-draft-repair-admission.ts`, a narrow policy module for repairing an existing owner Process draft whose only authoring gap is the unknown annual volume. It owns the candidate gate (bounded contract `save_draft` of `processes`, schema/content/multilingual passed, authoring failed with exactly `annual_supply_or_production_volume_missing`) and the content-bound decision against the fresh before image plus the exact candidate: both sides must keep `annualSupplyOrProductionVolume` as the unchanged empty array, and the only admissible delta is an existing `common:shortDescription.#text` under `dataSourcesTreatmentAndRepresentativeness.referenceToDataSource` or `publicationAndOwnership.common:referenceToOwnershipOfDataSet`; array lengths/order, languages, reference identities and every other field must be identical, at least one real change is required. `dataset-save-draft-run.ts` keeps the run-level ownership: it only admits a candidate after the fresh owner/state-0/before-hash read, re-validates the stored draft itself with the same real validator on a clone (so a before with any other error, a blank/placeholder reference description or a second authoring code is refused even when the candidate would fix it), dispatches the admitted row through the existing guarded before-image transport with `ruleVerification=false` (the sole exception; ordinary fully valid writes stay `true`), records `draft_repair_admission` on the row while preserving `validation.ok=false`, and gives a dry-run the same admission with zero dispatch. The verified admission is written into the first hashed `attempt_emitted` event (an optional field, so legacy attempt events stay valid) and the recovery paths return it from that original attempt, never re-deriving it from a fresh read that may already hold the desired content; a tampered, dropped or action-mismatched admission fails the ledger check. The same function now consults the attempt ledger before classifying a prepared failure, so a consumed attempt is reported from its ledger state (readback-only `executed`/`UNKNOWN`, or `blocked` with `retained_attempt`/`retained_outcome` in a dry-run) instead of an unconsumed `failed` row.

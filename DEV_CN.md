...
Review note, 2026-09-22: CLI #364 把版本化（v2）protected 批准绑定到唯一 canonical 批准时间戳。freeze 在自身时钟上指定 `approved_at_utc` 并写入 approval-request core（参与 `request_sha256`）与人类可见批准文本；`seal-protected-approval` 校验 `--approved-at` 必须与该值完全相等，缺失、非 canonical 或不同都会在产出批准前被拒（绑定前的旧 request 明确要求重新 freeze）。批准文本由一个模块内 renderer 依据 request 自身字段（时间戳、capability、plan/freeze 摘要、项目、计数）唯一重建，builder、parser 与 seal 共用该 helper，因此 root 的"自洽改时"攻击（改 `approved_at_utc`、重算 `request_sha256`、保留原批准文字与 text 摘要）在 parse 与 seal 两处都被拒绝：同一份批准文字无法再铸出第二个 approval identity。已封存 approval 的 14 键形状不变，绑定前封存的批准仍可解析并继续用于 status-only 恢复；服务端 preflight/admit/read 信封、expected 计数、一次性 runtime、v1、依赖、版本与 workflow 均不变，不新增数据库迁移或授权框架（本地 request 信封从不发送到数据库）。`src/**` 覆盖率保持 100%。

Review note, 2026-09-22: CLI #359 新增封闭的 **Length\*time** 修正 profile（`dataset-length-time-plan.v1`），作为版本化 protected 生命周期的第二个使用者。`plan --length-time-input` 由 reviewed 规划输入构建 plan：13 个 owner-draft Process action 的 39 个选中 occurrence 按固定因子 1000 移动其两个绝对量叶子（`kmy` → `m*a`，改动前两叶子字节相等），13 个只读 Product flow 仅作为 `{id, version, sha256}` 证据随 plan 传递，不产生任何 flow/text/no-op action。profile 只由 plan 文档自身的 `schema_version` 选择——Time plan 保留其 reviewed 判定、Length plan 使用自己的判定，互不放宽；共享的 freeze/approval/request/admit/read 信封、三项 gate、180 秒窗口与单次 admission 策略完全不变，freeze 仍为 13 键信封，Length plan 使用 wire 固定的 `{flowproperty, unitgroup}` target 投影。源编号只从真实语料的锚定声明读取（`Source EcoSpold1 exchange number: <N>.`，仅一次声明，后缀原样保留且从不扫描数字），不确定度两项为可选且逐字节保留（39 个 occurrence 中 13 个无分布、35 个无标准差），单位组因子按 exact decimal 比较（`1.0`、`1000.0`）。仓库内共享合成 cohort fixture 复现这些真实形状，两侧共享同一份 source comment 向量表。`src/**` 覆盖率保持 100%；生产使用仍需数据库侧对应能力与协同发布，未部署、未执行。CLI 侧已通过 root 的真实语料 plan oracle 与数据库 owner 对同一份产出文档的联合执行。
...
`plan --alias-v2-input` 只构建 `dataset-alias-plan.v2` plan 与 batch（不使用 `--scope`/`--operation`，但必须提供 `--out-dir`）；`freeze-protected` 在 v2 plan 上必须提供 `--derivative-baselines`（reviewed 六键 baseline，逐行覆盖 plan 实际改动的 387 行 Flow/Process 身份），并写出 `alias-v2-plan.json`、`alias-v2-batch.json`、`protected-v2-execution-freeze.json`、`protected-v2-approval-request.json`/`.txt`、`protected-v2-human-approval.txt`；`seal-protected-approval` 的字节级文本、freeze 文件 hash、request identity、text hash、账号核对与 v1 完全相同，批准时间核对与 v1 语义一致：freeze 在自己的时钟上指定唯一 canonical `approved_at_utc`，它既是 approval-request core 的字段（参与 `request_sha256`），也写在人类逐字批准的文本里，被命名为"request 指定的批准时间戳、与这些精确内容一同被接受、seal 必须原样复用"；`--approved-at` 必须与该值完全相等，缺失、非法或不同的时间戳都会在写出任何批准之前被拒绝（绑定前生成的旧 request 会被明确拒绝并要求重新 freeze，而不是被静默重读或改写）。批准文本由 request 自身字段（指定时间戳、capability、plan/freeze 摘要、项目与计数）唯一重建，必须逐字节一致，因此改动 request 时间戳或批准文字都无法得到 seal——同一份批准文字不可能再被换时间戳铸出第二个 approval identity。`run-protected` 由 seal 的 schema 决定链路——`dataset-alias-execution-freeze.v2` 走 v2，其它 seal 仍走冻结的 v1 链路，两种模式的 flag 与一次性 admission 语义不变。
